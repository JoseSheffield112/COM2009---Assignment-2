...
def main():
    ...
    us_right = ev3.UltrasonicSensor('in1')
    ls = ev3.ColorSensor('in2')
    ...
    while True:
        ...
        if( (us_left.value() < 100) or (us_right.value() < 100) ):
            ...
            debug_print("spiral")
            left_speed += 0.03
            m_left.run_direct(duty_cycle_sp=-left_speed)
            m_right.run_direct(duty_cycle_sp=-right_speed)  
            lightIntensity = ls.value()
            debug_print('light intensity: ', lightIntensity)
